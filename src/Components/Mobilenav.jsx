...
function Mobilenav({ menu, onToggle }) {
  ...
  useEffect(() => {
    ...
    // Additional side effects based on isOpen changes
  }, [isOpen]);

  const toggleMenu = () => {
    setIsOpen(prevIsOpen => !prevIsOpen);
    onToggle(); // If needed, propagate toggle action
  };
      ...
      >
        ...
        >
          <ul className='flex flex-col gap-[30px] items-center'>
            {menu.map((item, index) => (
              <li key={index}>
                <Link to={item} onClick={closeMenu} className='text-2xl capitalize'>{item}</Link>
              </li>
            ))}
          </ul>
          <ul className='flex flex-col items-center'>
            <li>
              <button onClick={closeMenu} className='text-secondary px-4 py-2 rounded border-secondary bg-[#42ADF8] text-white border-2 border-black'>
                Sign up
              </button>
            </li>
          </ul>
